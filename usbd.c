...
{
  if(UEINT & (1 << UEINT0))//(UEINT & (1 << EPINT0))
  {
    Usb_select_endpoint(0x00);
    bmRequestType_bmRequest  = UEDATX << 8;
    bmRequestType_bmRequest |= UEDATX;
    //if((bmRequestType_bmRequest != 0x8006) && (bmRequestType_bmRequest != 0x0005))
    //{
    //  bb = 44;
    //}
  ...
  if(UEINT & (1 << UEINT1))//(UEINT & (1 << EPINT1))
  {
  ...
  if(UEINT & (1 << UEINT2))//(UEINT & (1 << EPINT2))
  {
  ...
  if(UEINT & (1 << UEINT3))//(UEINT & (1 << EPINT3))
  {
    ;;
  }  
}
...
void USB_enable(void)
{
    ...
    //USBCON = (1<<USBE) | (1<<OTGPADE) | (1<<FRZCLK)|(1 << VBUSTE);
    //USBCON = ((1<<USBE) | (1<<OTGPADE) | (1<<FRZCLK)); // setting FRZCLK again in not necessary (see below)
    UDIEN = ((1<<EORSTE));//|(1<<WAKEUPE)); Включаем нужные прерывания
